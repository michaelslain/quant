...
def _stock_momentum_worker(closes_vals, volumes_vals, cols, rsi_np, vol_avg_np,
                           trend_np, kwargs, rebalance_every, initial_cash=100_000.0):
    """Standalone backtest function for multiprocessing (stock momentum)."""
...
class MomentumStrategy:
    """
    Stock momentum strategy with RSI, trend filter, and volume confirmation.
    Buys stocks showing strong upward momentum confirmed by volume
    and a longer-term trend filter (price above SMA).
    """

    GRID = {
        "lookback_minutes": [15, 30, 60],
        "rsi_period": [10, 14, 20],
    ...
    def __init__(self, api: tradeapi.REST, symbols: list[str],
                 lookback_minutes: int = 30, rsi_period: int = 14,
                 rsi_threshold: float = 55, volume_mult: float = 1.2,
                 trend_window: int = 120, top_n: int = 3):
        self.api = api
    ...
    def _load_params(self):
        ...
        if os.path.exists(pf):
            with open(pf) as f:
                p = json.load(f)
            self.lookback_minutes = p.get("lookback_minutes", self.lookback_minutes)
    ...
    def optimize(self, days: int = 7, fixed_interval: int = None,
        ...
        print(f"Optimizing stock momentum over {days} days of data...")
        ...
        for rebal in rebal_options:
            for vals in combos:
                ...
                jobs.append((closes_vals, volumes_vals, closes.columns.tolist(),
                             rsi_np, vol_avg_np, trend_np, kwargs, rebal))
                jobs_meta.append((kwargs, rebal))

        results = grid_search(_stock_momentum_worker, jobs)
        best_params, best_rebal, best_sharpe, best_return = find_best(jobs_meta, results)

        if best_params:
            self.lookback_minutes = best_params["lookback_minutes"]
            ...
            print(f"\n{label} params found:")
            print(f"  lookback_minutes={self.lookback_minutes}")
    ...
    def get_momentum_scores(self) -> pd.Series:
        ...
        lookback = max(self.rsi_period, self.trend_window, self.lookback_minutes) + 30
        ...
        for symbol in self.symbols:
            try:
                ...
                # Trend filter: price above trend SMA
    ...
    def rebalance(self):
        ...
        print(f"\n[{datetime.now()}] Running stock-momentum strategy...")
