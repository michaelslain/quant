...
def _stock_breakout_worker(closes_vals, highs_vals, volumes_vals, cols,
                           channel_np, vol_avg_np, atr_np, trend_np,
                           kwargs, rebalance_every, initial_cash=100_000.0):
    """Standalone backtest function for multiprocessing (stock breakout)."""
...
class BreakoutStrategy:
    """
    Stock channel breakout strategy.
    ...
    def optimize(self, days: int = 7, fixed_interval: int = None,
        ...
        print(f"Optimizing stock breakout over {days} days of data...")
        ...
        results = grid_search(_stock_breakout_worker, jobs)
    ...
    def get_momentum_scores(self) -> pd.Series:
        """Return breakout scores (matches interface name for rebalance loop)."""
        end = datetime.now()
    ...
    def rebalance(self):
        ...
        print(f"\n[{datetime.now()}] Running stock-breakout strategy...")
